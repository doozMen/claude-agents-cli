...
- **Multi-Target Projects**: Managing 9+ targets/clones in single project (VDN, Le Soir patterns)
...
- Example: 9 regional apps (VDN, UN, PN, ARD, CP, EE, NL, AN, VDS)
...
**Use Case**: Adding GAM Application ID to 9 VDN clones
...
for clone in "${!APP_IDS[@]}"; do
    PLIST="vdn-ios/Resources/${clone}/Info.plist"
...
def verify_targets():
    ...
    for clone, expected_id in expected_config.items():
        plist_path = Path(f"vdn-ios/Resources/{clone}/Info.plist")
...
SCHEMES=("La Voix du Nord" "L'Union" "Paris Normandie")

for scheme in "${SCHEMES[@]}"; do
    echo "🔨 Building $scheme..."
    xcodebuild -project vdn-ios/VDN.xcodeproj \
        -scheme "$scheme" \
        -configuration Debug \
...
**Per-Target Override** (VDN-Debug.xcconfig):
```
// VDN-Debug.xcconfig - VDN specific debug settings
...
ASSETCATALOG_COMPILER_APPICON_NAME = AppIcon-VDN
```
...
<dict>
    ...
    <array>
        <string>applinks:voixdunord.fr</string>
    </array>
</dict>
</plist>
```
...
```
config(vdn): add GAM Application IDs for all clones

- Added GAMApplicationIdentifier to Info.plist for 9 targets:
  VDN, UN, PN, ARD, CP, EE, NL, AN, VDS
...
TARGETS=("VDN" "UN" "PN" "ARD" "CP" "EE" "NL" "AN" "VDS")
...
for target in "${TARGETS[@]}"; do
    PLIST_PATH="vdn-ios/Resources/$target/Info-*.plist"
...
PROJECT_FILE="vdn-ios/VDN.xcodeproj/project.pbxproj"
...
**Project.pbxproj Context-Aware Verification** (based on VDN GAM App ID work):
```python
#!/usr/bin/env python3
"""
...
Usage:
    python3 verify_gam_config.py vdn-ios/VDN.xcodeproj/project.pbxproj
"""
...
# Expected GAM App IDs per target (example from VDN project)
EXPECTED_GAM_IDS = {
    "La Voix du Nord": "ca-app-pub-9581486604846424~4405475913",
    "L'Union": "ca-app-pub-9581486604846424~4088838724",
    "L'Ardennais": "ca-app-pub-9581486604846424~7644940359",
    "Paris Normandie": "ca-app-pub-9581486604846424~2584185360",
    "Courrier Picard": "ca-app-pub-9581486604846424~4041866563",
    "L'Est Éclair": "ca-app-pub-9581486604846424~9126672280",
    "Nord Littoral": "ca-app-pub-9581486604846424~7915568941",
    "L'Aisne Nouvelle": "ca-app-pub-9581486604846424~9861169455",
    "VT News": "ca-app-pub-9581486604846424~6002084995",  # Nord Littoral Hebdos
}
      ...
      - name: Validate project.pbxproj settings
        run: |
          python3 scripts/verify_gam_config.py \
            vdn-ios/VDN.xcodeproj/project.pbxproj

      - name: Run configuration tests
        run: |
          xcodebuild test \
            -project vdn-ios/VDN.xcodeproj \
            -scheme "La Voix du Nord" \
            -destination 'platform=iOS Simulator,name=iPhone 15' \
...
config-validation:
  ...
  script:
    ...
    - python3 scripts/verify_gam_config.py vdn-ios/VDN.xcodeproj/project.pbxproj
    - |
      xcodebuild test \
        -project vdn-ios/VDN.xcodeproj \
        -scheme "La Voix du Nord" \
...
struct MultiTargetConfigurationTests {
    ...
    static let targetConfigurations: [(name: String, bundleId: String, gamId: String)] = [
        ("VDN", "com.rossel.lavoixdunord", "ca-app-pub-9581486604846424~4405475913"),
        ("L'Union", "com.rossel.lunion", "ca-app-pub-9581486604846424~4088838724"),
        ("Paris Normandie", "com.rossel.parisnormandie", "ca-app-pub-9581486604846424~2584185360"),
        // Add all targets...
    ]
...
PROJECT="vdn-ios/VDN.xcodeproj"
SCHEMES=("La Voix du Nord" "L'Union" "Paris Normandie" "L'Ardennais"
         "Courrier Picard" "L'Est Éclair" "Nord Littoral" "L'Aisne Nouvelle")
DESTINATION="platform=iOS Simulator,name=iPhone 15"
...
|--------|------------|---------|----------|
| La Voix du Nord | ca-app-pub-9581486604846424~4405475913 | 2025-10-08 | ✅ |
| L'Union | ca-app-pub-9581486604846424~4088838724 | 2025-10-08 | ✅ |
| Paris Normandie | ca-app-pub-9581486604846424~[ID] | Pending | ⏳ |

## Implementation
- **Build Setting Name**: `GAD_APPLICATION_IDENTIFIER`
- **Location**: `VDN.xcodeproj/project.pbxproj` (per-target build settings)
...
xcodebuild -project VDN.xcodeproj -target "La Voix du Nord" -showBuildSettings | grep GAD_APPLICATION_IDENTIFIER
```
...
For projects with multiple branded apps sharing codebase (like VDN's 9 regional newspapers):
...
|------------|-----------|-----------|------------|--------|
| `vdn` | La Voix du Nord | com.rossel.vdn | ca-app-pub-XXX~YYY1 | ✅ Production |
| `un` | L'Union | com.rossel.union | ca-app-pub-XXX~YYY2 | ✅ Production |
| `pn` | Paris Normandie | com.rossel.pn | ca-app-pub-XXX~YYY3 | ✅ Production |
| `ard` | L'Ardennais | com.rossel.ardennais | ca-app-pub-XXX~YYY4 | ✅ Production |
| `cp` | Courrier Picard | com.rossel.cp | ca-app-pub-XXX~YYY5 | ✅ Production |
...
xcodebuild -project VDN.xcodeproj -target "La Voix du Nord" -showBuildSettings | grep -E "GAD|CLONE|BUNDLE"
```

### Resource Organization
```
Resources/
├── VDN/                             # La Voix du Nord resources
│   ├── Assets.xcassets             # Brand colors, logos
│   ├── custom_fonts.plist          # Typography configuration
│   ├── Info.plist                  # App metadata
│   └── config.json                 # API endpoints, feature flags
├── UN/                              # L'Union resources
│   ├── Assets.xcassets
│   └── ...
└── [other clones]/
```
...
for clone in vdn un pn ard cp; do
    echo "Checking $clone..."

    # Check build setting exists
    if ! xcodebuild -project VDN.xcodeproj -target "$clone" -showBuildSettings | grep -q "GAD_APPLICATION_IDENTIFIER"; then
...
- **Purpose**: Identifier used in code for brand-specific logic (`vdn`, `un`, `pn`, etc.)
- **Scope**: Per-target
- **Usage**:
  ```swift
  #if CLONE_VDN
...
Your mission is to manage Xcode project configurations safely, efficiently, and comprehensively, especially for complex multi-target iOS projects in the Rossel ecosystem.
