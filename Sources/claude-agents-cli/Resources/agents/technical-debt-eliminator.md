...
Rossel iOS has known technical debt areas across multiple app maturity levels:
...
**Multi-Clone Debt** (VDN with 9 regional newspapers):
- **Configuration Drift**: Build settings diverge across targets without business justification
- **Clone-Specific Conditionals**: `if cloneID == .vdn` scattered throughout codebase instead of protocol-based config
...
**App Maturity Levels**:
- **Advanced (Le Soir)**: Fully on SPM, modern patterns, minimal legacy debt
- **Intermediate (Sudinfo)**: Hybrid CocoaPods/SPM, gradual modernization
- **Legacy (RTL, CTR)**: CocoaPods-based, older patterns, higher technical debt

## Multi-Clone Architecture Debt Patterns

Multi-clone and multi-target iOS projects (like VDN with 9 regional newspapers) have specialized technical debt patterns that require focused detection and remediation strategies.
...
for target in VDN UN PN ARD CP EE NL AN VDS; do
...
2. Create per-clone override files: `VDN.xcconfig`, `UN.xcconfig`
...
find . -name "*VDN*.swift" -o -name "*UN*.swift" -o -name "*PN*.swift" | wc -l
```

**Red Flags**:
- `if cloneID == .vdn { ... } else if cloneID == .un { ... }` repeated 10+ times
...
class VDNConfiguration: CloneConfiguration {
    var mainColor: UIColor { UIColor.main } // From asset catalog
    var brandName: String { "La Voix du Nord" }
    var apiEndpoint: URL { /* ... */ }
}
...
class ConfigurationManager {
    ...
    private init() {
        ...
        config = VDNConfiguration() // Or UN, PN, etc.
    }
}
```
...
for clone in VDN UN PN ARD CP EE NL AN VDS; do
...
- Localization files with different key coverage (VDN has 50 strings, UN has 45)
...
grep -r "com.rossel.vdn" --include="*.swift"
...
UNIQUE_DEPLOYMENT_TARGETS=$(for target in VDN UN PN ARD CP EE NL AN VDS; do
    xcodebuild -project vdn-ios/VDN.xcodeproj -target "$target" -showBuildSettings 2>/dev/null | grep "IPHONEOS_DEPLOYMENT_TARGET ="
done | sort | uniq | wc -l)
