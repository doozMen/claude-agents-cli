...
When multiple branded apps (clones) share a single codebase, careful architecture prevents code duplication while enabling per-brand customization. Regional App 1 iOS (9 regional newspapers) exemplifies this pattern in the CompanyA ecosystem.
...
**Regional App 1 Example**: 9 regional newspapers share editorial features, analytics, ads, and navigation but differ in branding (colors, fonts, logos) and content sources.
...
│   ├── Regional App 1/                       # Clone 1 - La Regional App 1
│   │   ├── Info.plist
│   │   ├── Assets.xcassets
│   │   │   ├── AppIcon
...
│   ├── UN/                        # Clone 2 - Regional App 2
│   ├── PN/                        # Clone 3 - Regional App 3
│   └── [7 more clones...]
├── Shared/                         # Shared resources across all clones
│   ├── Fonts/                     # Font files (.ttf, .otf)
│   └── CommonAssets.xcassets
├── companya-library-ios/            # KMM integration layer
...
// Regional App 1Configuration.swift (in Resources/Regional App 1/)
struct Regional App 1Configuration: CloneConfiguration {
    let identifier = "Regional App 1"
    let displayName = "La Regional App 1"

    let colorScheme = CloneColorScheme(
        primary: .regional-app-1Primary,
        secondary: .regional-app-1Secondary,
        accent: .regional-app-1Accent
    )
    ...
    let apiConfiguration = APIConfiguration(
        baseURL: "https://api.laregional-app-1.example",
        analyticsKey: "regional-app-1_analytics_key"
    )
}
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    static let configuration: CloneConfiguration = {
        ...
        return Regional App 1Configuration()
        #elseif CLONE_UN
...
// Resources/Regional App 1/config.json
{
...
// Resources/Regional App 1/custom_fonts.plist
...
**Xcode Scheme Setup**:
1. One scheme per clone: "La Regional App 1", "Regional App 2", "Regional App 3", etc.
2. Each scheme targets specific build configuration: `Regional App 1-Debug`, `Regional App 1-Release`
3. Environment variables set per scheme:
   - `CLONE_IDENTIFIER=Regional App 1`
   - `CLONE_DISPLAY_NAME=La Regional App 1`
...
platform :ios do
  ...
  lane :deploy_clone do |options|
    clone = options[:clone] # 'regional-app-1', 'un', 'pn', etc.
  ...
  def scheme_for_clone(clone)
    {
      'regional-app-1' => 'La Regional App 1',
      'un' => 'L\'Union',
      'pn' => 'Regional App 3'
    }[clone]
  end
...
workflows:
  deploy_clone:
    ...
    - script:
        ...
        - content: |
            #!/bin/bash
            # Tag format: regional-app-1/6.0.0/2 → clone=regional-app-1, version=6.0.0, build=2
    ...
    - deploy-to-itunesconnect:
        inputs:
        - bundle_id: com.companya.$CLONE
```

**Tag Format**: `[clone]/[version]/[build]` → `regional-app-1/6.0.0/2`, `un/5.1.0/10`
...
```swift
// Regional App 1Tests/Regional App 1ConfigurationTests.swift
// Only runs for Regional App 1 target
@Suite("Regional App 1 Configuration Tests")
struct Regional App 1ConfigurationTests {
    @Test("Regional App 1 configuration has correct bundle ID")
    func testBundleIdentifier() {
        let config = Regional App 1Configuration()
        #expect(config.identifier == "Regional App 1")
    }

    @Test("Regional App 1 color scheme is correct")
    func testColorScheme() {
        let colors = Regional App 1Configuration().colorScheme
        #expect(colors.primary == .regional-app-1Primary)
    }
}
```
...
│   ├── ServiceTests/
│   └── ViewModelTests/
├── Regional App 1Tests/               # Regional App 1-specific tests (target: Regional App 1 only)
│   ├── Regional App 1ConfigurationTests.swift
│   └── Regional App 1UITests.swift
├── UNTests/                # Regional App 2-specific tests
└── PNTests/                # Regional App 3-specific tests
```
...
- **Solution**: Namespace assets by clone (`Regional App 1Logo`, `UNLogo`) OR use separate asset catalogs per target
...
### Real-World Example: Regional App 1 iOS

Regional App 1 iOS successfully manages **9 regional newspaper apps** with this architecture:

**Shared Code (85%)**:
- Editorial content fetching (CompanyA Libraries KMM)
...
git tag regional-app-1/6.0.0/2
git push origin regional-app-1/6.0.0/2
# → Builds only Regional App 1 target, uploads to App Store Connect
```
