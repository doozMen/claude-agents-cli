...
|-----|--------------|----------------|------------|---------|-------|
| LeSoir | L3 | 15% | 1.89 | 283 | 150 |
| Sudinfo | L3 | 18% | 2.18 | 109 | 50 |
| RTL | L1 | 60% | 2.01 | 562 | 280 |
| CTR | L1 | 65% | 2.18 | 109 | 50 |
...
```markdown
## Modernization ROI Forecast: RTL (L1 → L3)
...
- **Expected Crash Rate**: 1.89 crashes/user (based on LeSoir benchmark)
...
**Examples**:
- `be.rossel.lesoiriphone` → `be_rossel_lesoiriphone_IOS`
- `be.rtl.newrtlinfo` → `be_rtl_newrtlinfo_IOS`
...
```
"Forecast crash reduction if RTL migrates from L1 to L3 based on LeSoir crash rates"
```
...
**Convergence Tracking**:
```
"Compare crash rates before and after Sudinfo's SPM migration (L2 → L3 transition)"
```
...
- **firebase-rossel-ecosystem-analyzer**: Cross-app Firebase data analysis
